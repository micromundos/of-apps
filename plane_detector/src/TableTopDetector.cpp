...
void TableTopDetector<PointT> :: initialize()
{
  ...
  segmentor_.setMaxIterations (1000);
...
bool TableTopDetector<PointT> :: detect(pcl::PointCloud<Point>& cloud)
{
  ...
  if ( ! findTableObjects() ) return false;
  if ( ! splitTableObjects() ) return false;
  return true;
}
...
bool TableTopDetector<PointT> :: findTable()
{
  ...
  printf("\t\tNumber of table plane cloud points: %d.\n", (int)table_cloud_->points.size ());
