...
void ofApp::draw()
{
  ...
  if ( kinect_img.isAllocated() )
    kinect_img.draw(10,10,kinect_img.getWidth(),kinect_img.getHeight());
...
void ofApp::update_kinect_data()
{
  while( receiver.hasWaitingMessages() )
  {
    ofxOscMessage m;
    receiver.getNextMessage(&m);
    ofLog() << "receive osc, addr: " << m.getAddress();
    if( m.getAddress() == "/depth" )
    {
      ofBuffer buffer = m.getArgAsBlob(0);
      kinect_img.loadImage(buffer);
      ofLog() << "\t" 
        << ", image alloc: " << kinect_img.isAllocated()
        << ", buf size: " << buffer.getText().size();
    }
  }
}
...
void ofApp::update_mesh()
{
  ...
  int w = kinect_img.getWidth();
  int h = kinect_img.getHeight();
  int step = 1;
