...
void main() 
{
    ...
    avg += texture2D( tex0, loc ) * gauss.x;
    coeffsum += gauss.x;
    gauss.xy *= gauss.yz;
    ...
    { 
        avg += texture2D( tex0, loc - i * size * dir ) * gauss.x;
        avg += texture2D( tex0, loc + i * size * dir ) * gauss.x;
...
/*{*/
    ...
    /*acc = texture2D( tex0, loc ); // accumulate center pixel*/

    /*for ( int i = 1; i <= kernel; i++ ) */
    /*{*/
        /*float coeff = exp(-0.5 * float(i) * float(i) / (sigma * sigma));*/
        /*acc += (texture2D(tex0, loc - float(i) * dir)) * coeff;//L*/
        /*acc += (texture2D(tex0, loc + float(i) * dir)) * coeff;//R*/
    /*}*/
